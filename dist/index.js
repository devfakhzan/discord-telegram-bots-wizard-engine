  ...
  switch (step.type) {
    ...
    case "input":
      if (step.valueType === "dateRange") {
        ...
        return `${(0, import_dayjs.default)(from).format(dateTimeFormat)} - ${(0, import_dayjs.default)(to).format(
          dateTimeFormat
        )}`;
      }
      return value;
                ...
                if (!date.isValid()) {
                  return await ctx.reply(
                    ctx.i18next.t("error.invalidFutureDateFormat", {
                      exampleDate: (0, import_dayjs3.default)().add(1, "day").format(dateTimeFormat)
                    })
                  );
                }
