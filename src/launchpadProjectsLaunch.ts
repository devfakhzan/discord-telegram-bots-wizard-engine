  ...
  arr = arr.slice(
    ...
    steps.filter(
      (s: {
        step: any;
        title: {
          includes: (arg0: string) => { (): any; new (): any; length: any };
        };
      }) => !s.step.includes("DONESTEP")
    ).length
  );
...
export const readObject = (obj: any, path: string, mainSteps: any) => {
    ...
    case "select":
      ...
      return options.reduce(
        (
          ...
          currentVal: MultiOptionObject
        ) => {
          if (Array.isArray(o)) {
            const val = o.find((oo: MultiOptionObject) => oo.value === value);
            if (val) {
              return val;
            }
            return currentVal;
...
export const getSummary = async (
  ...
  for (let mainStep of usedMainSteps) {
    ...
    for (let step of mainStep.steps) {
      ...
      if (
        step?.excludeFromSummaryIfValueIs &&
        step?.excludeFromSummaryIfValueIs ===
          readObject(obj, step.mapTo, mainSteps)
      ) {
        continue;
